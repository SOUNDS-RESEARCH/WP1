function[X_postproc] = MD_NINT(TDOA,X,tau,STFT_params)
...
%1. Compute remaining time-domain TDOA compensation filter
...
%optional normalization (used to match normalization done to speech
%signals in earlier step)
...
function phi_term = comp_phi(lFrame,nFreq_filter,window)
    ...
    window_sum_m_term = zeros(nFreq_filter,nFreq_filter,2*lFrame-1);

    k_dash_offset_range = -(nFreq_filter - 1)/2:(nFreq_filter - 1)/2;
    ...
    for k = 1:nFreq_filter
        ...
        window_sum_m_term(k,k_dash_indexs,:) = conv2(window.*exp(-1*1i*((2*pi)/lFrame)*(0:lFrame-1).'*k_dash_offset_range_temp),window).';
    end
    
    phi_term = zeros(size(window_sum_m_term));
    
    for k = 1:nFreq_filter
        phi_term(k,:,:) = squeeze(window_sum_m_term(k,:,:)).*e_term;
    end
end
...
function u_m = comp_u_m(td_filter,phi_term,causal_shift_total,L_shift,nChan,nFreq_filter)
    ...
    u_m = zeros(nFreq_filter,nFreq_filter,n_taps,nChan);
    for i = 1:nChan
        td_filter_reshape = reshape(td_filter(:,i),[1,1,size(td_filter,1)]);
        h_term = convn(phi_term,td_filter_reshape);   
